...
_T: dict[str, dict[str, str]] = {
    ...
    "mapa.mostrar_sensoriamento": {"pt": "Mostrar pontos de sensoriamento remoto", "en": "Show remote sensing points", "fr": "Afficher les points de télédétection", "es": "Mostrar puntos de teledetección"},
    "mapa.sensoriamento_observado": {"pt": "Landsat real (1984–presente)", "en": "real Landsat (1984–present)", "fr": "Landsat réel (1984–présent)", "es": "Landsat real (1984–presente)"},
    "mapa.sensoriamento_simulado": {"pt": "planilha ilustrativa (simulado)", "en": "illustrative spreadsheet (simulated)", "fr": "tableur illustratif (simulé)", "es": "hoja ilustrativa (simulada)"},
    "mapa.sensoriamento_misto": {"pt": "real + ilustrativo (misto)", "en": "real + illustrative (mixed)", "fr": "réel + illustratif (mixte)", "es": "real + ilustrativo (mixto)"},
    "mapa.sensoriamento_ano_ref": {"pt": "ano de referência", "en": "reference year", "fr": "année de référence", "es": "año de referencia"},
    "mapa.sensoriamento_sem_dado": {"pt": "sem dado de sensoriamento", "en": "no sensing data", "fr": "aucune donnée de télédétection", "es": "sin dato de teledetección"},
    "mapa.sensoriamento_ressalva": {
        "pt": "Índices Landsat (NDVI, proxy de turbidez, temperatura) ainda não calibrados contra medição in situ do Tietê — tratar como indicativo, não substituto da medição de campo.",
        "en": "Landsat indices (NDVI, turbidity proxy, temperature) not yet calibrated against in-situ Tietê measurements — treat as indicative, not a substitute for field measurement.",
        "fr": "Indices Landsat (NDVI, proxy de turbidité, température) pas encore calibrés par rapport aux mesures in situ du Tietê — à considérer comme indicatifs, pas comme substituts à la mesure de terrain.",
        "es": "Índices Landsat (NDVI, proxy de turbidez, temperatura) aún no calibrados con mediciones in situ del Tietê — tratar como indicativo, no como sustituto de la medición de campo.",
    },
    ...
    "rel.b.resumo_texto": {
        "pt": "Este relatório documenta o estado da qualidade da água do trecho {trecho} do Rio Tietê no ano de {ano}, com base em medições reais da rede CETESB (1978-2024, quando disponíveis para o trecho/ano), complementadas por uma série simulada de preenchimento nos anos sem essa cobertura. O Índice de Qualidade da Água (ICA) foi de {iqa:.1f} pontos, em situação **{status}**, {comparacao} média histórica da série (1940-{ano_max}) de {media:.1f}. O Oxigênio Dissolvido está em {od:.2f} mg/L e a Demanda Bioquímica de Oxigênio em {dbo:.1f} mg/L. Nos últimos {janela} anos, o ICA apresentou tendência de {dir_iqa} ({delta_iqa:+.1f} pontos). Conclui-se que o trecho {trecho} {conclusao_curta}.",
        "en": "This report documents the water quality status of the {trecho} stretch of the Tietê River in {ano}, based on real CETESB network measurements (1978-2024, where available for the stretch/year), complemented by a simulated series as fill-in for years without that coverage. The Water Quality Index (WQI) was {iqa:.1f} points, in **{status}** condition, {comparacao} the series' historical average (1940-{ano_max}) of {media:.1f}. Dissolved Oxygen is at {od:.2f} mg/L and Biochemical Oxygen Demand at {dbo:.1f} mg/L. Over the last {janela} years, the WQI trend was {dir_iqa} ({delta_iqa:+.1f} points). It is concluded that the {trecho} stretch {conclusao_curta}.",
        "fr": "Ce rapport documente l'état de la qualité de l'eau du tronçon {trecho} du fleuve Tietê en {ano}, sur la base de mesures réelles du réseau CETESB (1978-2024, lorsque disponibles pour le tronçon/année), complétées par une série simulée en remplissage pour les années sans cette couverture. L'Indice de Qualité de l'Eau (IQE) était de {iqa:.1f} points, en situation **{status}**, {comparacao} la moyenne historique de la série (1940-{ano_max}) de {media:.1f}. L'Oxygène Dissous est à {od:.2f} mg/L et la Demande Biochimique en Oxygène à {dbo:.1f} mg/L. Au cours des {janela} dernières années, l'IQE a montré une tendance {dir_iqa} ({delta_iqa:+.1f} points). Il est conclu que le tronçon {trecho} {conclusao_curta}.",
        "es": "Este informe documenta el estado de la calidad del agua del tramo {trecho} del Río Tietê en el año {ano}, con base en mediciones reales de la red CETESB (1978-2024, cuando disponibles para el tramo/año), complementadas por una serie simulada como relleno en los años sin esa cobertura. El Índice de Calidad del Agua (ICA) fue de {iqa:.1f} puntos, en situación **{status}**, {comparacao} promedio histórico de la serie (1940-{ano_max}) de {media:.1f}. El Oxígeno Disuelto está en {od:.2f} mg/L y la Demanda Bioquímica de Oxígeno en {dbo:.1f} mg/L. En los últimos {janela} años, el ICA presentó una tendencia {dir_iqa} ({delta_iqa:+.1f} puntos). Se concluye que el tramo {trecho} {conclusao_curta}.",
    },
    ...
    "rel.b.objetivo_geral_texto": {
        "pt": "Analisar o estado da qualidade da água do trecho {trecho} do Rio Tietê no ano de {ano}, a partir dos indicadores de Índice de Qualidade da Água, Oxigênio Dissolvido e Demanda Bioquímica de Oxigênio (medição real da CETESB, complementada por série simulada nos anos sem cobertura real).",
        "en": "Analyze the water quality status of the {trecho} stretch of the Tietê River in {ano}, based on the Water Quality Index, Dissolved Oxygen and Biochemical Oxygen Demand indicators (real CETESB measurement, complemented by a simulated series for years without real coverage).",
        "fr": "Analyser l'état de la qualité de l'eau du tronçon {trecho} du fleuve Tietê en {ano}, à partir des indicateurs d'Indice de Qualité de l'Eau, d'Oxygène Dissous et de Demande Biochimique en Oxygène (mesure réelle CETESB, complétée par une série simulée pour les années sans couverture réelle).",
        "es": "Analizar el estado de la calidad del agua del tramo {trecho} del Río Tietê en el año {ano}, a partir de los indicadores de Índice de Calidad del Agua, Oxígeno Disuelto y Demanda Bioquímica de Oxígeno (medición real de la CETESB, complementada por serie simulada en los años sin cobertura real).",
    },
    ...
    "rel.b.objetivo_geral_texto_todos": {
        "pt": "Analisar o estado da qualidade da água de todos os trechos monitorados do Rio Tietê no ano de {ano}, a partir dos indicadores de Índice de Qualidade da Água, Oxigênio Dissolvido e Demanda Bioquímica de Oxigênio (medição real da CETESB, complementada por série simulada nos anos sem cobertura real).",
        "en": "Analyze the water quality status of all monitored stretches of the Tietê River in {ano}, based on the Water Quality Index, Dissolved Oxygen and Biochemical Oxygen Demand indicators (real CETESB measurement, complemented by a simulated series for years without real coverage).",
        "fr": "Analyser l'état de la qualité de l'eau de tous les tronçons surveillés du fleuve Tietê en {ano}, à partir des indicateurs d'Indice de Qualité de l'Eau, d'Oxygène Dissous et de Demande Biochimique en Oxygène (mesure réelle CETESB, complétée par une série simulée pour les années sans couverture réelle).",
        "es": "Analizar el estado de la calidad del agua de todos los tramos monitoreados del Río Tietê en el año {ano}, a partir de los indicadores de Índice de Calidad del Agua, Oxígeno Disuelto y Demanda Bioquímica de Oxígeno (medición real de la CETESB, complementada por serie simulada en los años sin cobertura real).",
    },
    ...
    "rel.b.resumo_texto_todos": {
        "pt": "Este relatório documenta o estado da qualidade da água de todos os trechos monitorados do Rio Tietê no ano de {ano}, com base em medições reais da rede CETESB (1978-2024, quando disponíveis para o trecho/ano), complementadas por uma série simulada de preenchimento nos anos sem essa cobertura. No ano de referência, {resumo_trechos}. Conclui-se que a bacia apresenta condições heterogêneas entre trechos, refletindo diferentes pressões de poluição ao longo do curso do rio, o que reforça a importância do acompanhamento segmentado por trecho.",
        "en": "This report documents the water quality status of all monitored stretches of the Tietê River in {ano}, based on real CETESB network measurements (1978-2024, where available for the stretch/year), complemented by a simulated series as fill-in for years without that coverage. In the reference year, {resumo_trechos}. It is concluded that the basin presents heterogeneous conditions across stretches, reflecting different pollution pressures along the river's course, which reinforces the importance of stretch-by-stretch monitoring.",
        "fr": "Ce rapport documente l'état de la qualité de l'eau de tous les tronçons surveillés du fleuve Tietê en {ano}, sur la base de mesures réelles du réseau CETESB (1978-2024, lorsque disponibles pour le tronçon/année), complétées par une série simulée en remplissage pour les années sans cette couverture. Pour l'année de référence, {resumo_trechos}. Il est conclu que le bassin présente des conditions hétérogènes entre les tronçons, reflétant différentes pressions de pollution le long du cours du fleuve, ce qui renforce l'importance d'un suivi segmenté par tronçon.",
        "es": "Este informe documenta el estado de la calidad del agua de todos los tramos monitoreados del Río Tietê en el año {ano}, con base en mediciones reales de la red CETESB (1978-2024, cuando disponibles para el tramo/año), complementadas por una serie simulada como relleno en los años sin esa cobertura. En el año de referencia, {resumo_trechos}. Se concluye que la cuenca presenta condiciones heterogéneas entre tramos, reflejando diferentes presiones de contaminación a lo largo del curso del río, lo que refuerza la importancia del seguimiento segmentado por tramo.",
    },
    "rel.b.conclusao_texto_todos": {
        "pt": "Com base nos indicadores analisados, os trechos do Rio Tietê apresentam condições heterogêneas de qualidade da água, refletindo diferentes pressões de poluição ao longo do curso do rio. Como limitação, destaca-se que parte do histórico (1940-1977 e lacunas pontuais) é preenchida por uma série simulada (proxy histórico), não por medição direta — os demais anos (1978-2024) vêm de medição real da rede CETESB —, e que cada trecho é reportado de forma independente. Recomenda-se o acompanhamento contínuo por trecho e a priorização de medidas de controle nos trechos em situação mais crítica.",
        "en": "Based on the analyzed indicators, the stretches of the Tietê River present heterogeneous water quality conditions, reflecting different pollution pressures along the river's course. As a limitation, it should be noted that part of the historical record (1940-1977 and occasional gaps) is filled by a simulated series (historical proxy), not direct measurement — the remaining years (1978-2024) come from real CETESB network measurement — and that each stretch is reported independently. Continuous stretch-by-stretch monitoring is recommended, along with prioritizing control measures in the stretches with the most critical condition.",
        "fr": "Sur la base des indicateurs analysés, les tronçons du fleuve Tietê présentent des conditions hétérogènes de qualité de l'eau, reflétant différentes pressions de pollution le long du cours du fleuve. Comme limite, il convient de noter qu'une partie de l'historique (1940-1977 et lacunes ponctuelles) est comblée par une série simulée (proxy historique), pas par une mesure directe — les autres années (1978-2024) proviennent de mesures réelles du réseau CETESB —, et que chaque tronçon est rapporté indépendamment. Un suivi continu par tronçon est recommandé, ainsi que la priorisation de mesures de contrôle dans les tronçons en situation la plus critique.",
        "es": "Con base en los indicadores analizados, los tramos del Río Tietê presentan condiciones heterogéneas de calidad del agua, reflejando diferentes presiones de contaminación a lo largo del curso del río. Como limitación, se destaca que parte del histórico (1940-1977 y brechas puntuales) se completa con una serie simulada (proxy histórico), no con medición directa — los demás años (1978-2024) provienen de medición real de la red CETESB —, y que cada tramo se reporta de forma independiente. Se recomienda el seguimiento continuo por tramo y la priorización de medidas de control en los tramos en situación más crítica.",
    },
    "rel.b.metodologia_intro": {
        "pt": "Os indicadores reportados a seguir vêm de medições reais da rede CETESB, complementadas por uma série simulada de preenchimento nos anos sem essa cobertura, conforme descrito a seguir.",
        "en": "The indicators reported below come from real CETESB network measurements, complemented by a simulated series as fill-in for years without that coverage, as described below.",
        "fr": "Les indicateurs rapportés ci-dessous proviennent de mesures réelles du réseau CETESB, complétées par une série simulée en remplissage pour les années sans cette couverture, comme décrit ci-dessous.",
        "es": "Los indicadores reportados a continuación provienen de mediciones reales de la red CETESB, complementadas por una serie simulada como relleno en los años sin esa cobertura, como se describe a continuación.",
    },
    # ACHADO (2026-09): `narrative_generator.gerar_relatorio_trecho_completo` reaproveitava
    # `cn.nota` (a metodologia do ABM/Streeter-Phelps, correta para o relatório de "Cenários
    # Futuros") como corpo da metodologia deste relatório — que na verdade usa
    # `load_qualidade_historica` (medição REAL da CETESB + fallback simulado), nunca o ABM.
    # Esta chave é a descrição correta, usada no lugar de `cn.nota` a partir da correção.
    "rel.b.metodologia_corpo": {
        "pt": "Os indicadores vêm de medições reais da rede CETESB (1978-2024, quando disponíveis para o trecho/ano); nos anos sem essa cobertura (1940-1977 e lacunas pontuais) usa-se uma série simulada (proxy histórico) como preenchimento — ver `transform.gold_features.qualidade_real_com_fallback_simulado`. O IQA é um proxy simplificado a partir de OD/DBO (0,6 x OD/8mg/L + 0,4 x (100 - DBO/10mg/L)), não o índice oficial CETESB/NSF de 9 parâmetros.",
        "en": "The indicators come from real CETESB network measurements (1978-2024, where available for the stretch/year); years without that coverage (1940-1977 and occasional gaps) fall back to a simulated series (historical proxy) as fill-in — see `transform.gold_features.qualidade_real_com_fallback_simulado`. The WQI is a simplified proxy from DO/BOD (0.6 x DO/8mg/L + 0.4 x (100 - BOD/10mg/L)), not the official 9-parameter CETESB/NSF index.",
        "fr": "Les indicateurs proviennent de mesures réelles du réseau CETESB (1978-2024, lorsque disponibles pour le tronçon/année) ; les années sans cette couverture (1940-1977 et lacunes ponctuelles) utilisent une série simulée (proxy historique) en remplissage — voir `transform.gold_features.qualidade_real_com_fallback_simulado`. L'IQE est un proxy simplifié à partir de l'OD/DBO (0,6 x OD/8mg/L + 0,4 x (100 - DBO/10mg/L)), pas l'indice officiel CETESB/NSF à 9 paramètres.",
        "es": "Los indicadores provienen de mediciones reales de la red CETESB (1978-2024, cuando disponibles para el tramo/año); los años sin esa cobertura (1940-1977 y brechas puntuales) usan una serie simulada (proxy histórico) como relleno — ver `transform.gold_features.qualidade_real_com_fallback_simulado`. El ICA es un proxy simplificado a partir de OD/DBO (0,6 x OD/8mg/L + 0,4 x (100 - DBO/10mg/L)), no el índice oficial CETESB/NSF de 9 parámetros.",
    },
    "rel.b.desenvolvimento_intro": {
        "pt": "Apresentam-se a seguir os indicadores do trecho {trecho} no ano de {ano}.",
        "en": "The indicators for the {trecho} stretch in {ano} are presented below.",
        "fr": "Les indicateurs du tronçon {trecho} pour l'année {ano} sont présentés ci-dessous.",
        "es": "A continuación se presentan los indicadores del tramo {trecho} en el año {ano}.",
    },
    "rel.b.conclusao_texto": {
        "pt": "Com base nos indicadores analisados, o trecho {trecho} encontrava-se em situação **{status}**, com tendência de {dir_iqa} ao longo dos últimos {janela} anos. Como limitação, destaca-se que parte do histórico (1940-1977 e lacunas pontuais) é preenchida por uma série simulada (proxy histórico), não por medição direta — os demais anos (1978-2024) vêm de medição real da rede CETESB. Recomenda-se o acompanhamento contínuo dos parâmetros de Oxigênio Dissolvido e Demanda Bioquímica de Oxigênio, e a priorização de medidas de controle caso a tendência de deterioração se confirme.",
        "en": "Based on the analyzed indicators, the {trecho} stretch was in **{status}** condition, with a {dir_iqa} trend over the last {janela} years. As a limitation, it should be noted that part of the historical record (1940-1977 and occasional gaps) is filled by a simulated series (historical proxy), not direct measurement — the remaining years (1978-2024) come from real CETESB network measurement. Continuous monitoring of the Dissolved Oxygen and Biochemical Oxygen Demand parameters is recommended, along with prioritizing control measures should the deteriorating trend be confirmed.",
        "fr": "Sur la base des indicateurs analysés, le tronçon {trecho} se trouvait en situation **{status}**, avec une tendance {dir_iqa} au cours des {janela} dernières années. Comme limite, il convient de noter qu'une partie de l'historique (1940-1977 et lacunes ponctuelles) est comblée par une série simulée (proxy historique), pas par une mesure directe — les autres années (1978-2024) proviennent de mesures réelles du réseau CETESB. Il est recommandé de suivre en continu les paramètres d'Oxygène Dissous et de Demande Biochimique en Oxygène, et de prioriser des mesures de contrôle si la tendance à la détérioration se confirme.",
        "es": "Con base en los indicadores analizados, el tramo {trecho} se encontraba en situación **{status}**, con tendencia {dir_iqa} a lo largo de los últimos {janela} años. Como limitación, se destaca que parte del histórico (1940-1977 y brechas puntuales) se completa con una serie simulada (proxy histórico), no con medición directa — los demás años (1978-2024) provienen de medición real de la red CETESB. Se recomienda el seguimiento continuo de los parámetros de Oxígeno Disuelto y Demanda Bioquímica de Oxígeno, y la priorización de medidas de control en caso de confirmarse la tendencia de deterioro.",
    },
    "rel.b.anexos_texto": {
        "pt": "Os dados brutos do histórico de qualidade da água (todos os anos e trechos) podem ser consultados nas telas \"Séries Históricas\" e \"Mapa Interativo\" do painel WaterWeave-Tietê.",
        "en": "The raw data of the water quality history (all years and stretches) can be consulted on the \"Historical Series\" and \"Interactive Map\" screens of the WaterWeave-Tietê dashboard.",
        "fr": "Les données brutes de l'historique de qualité de l'eau (toutes les années et tous les tronçons) peuvent être consultées sur les écrans « Séries Historiques » et « Carte Interactive » du tableau de bord WaterWeave-Tietê.",
        "es": "Los datos brutos del histórico de calidad del agua (todos los años y tramos) pueden consultarse en las pantallas \"Series Históricas\" y \"Mapa Interactivo\" del panel WaterWeave-Tietê.",
    },
    ...
    "pdf.a.objetivo_texto": {
        "pt": "Apresentar, de forma direta e objetiva, o estado atual da qualidade da água do trecho {trecho} do Rio Tietê no ano de {ano}, com base em medições reais da rede CETESB, complementadas por uma série simulada de preenchimento nos anos sem essa cobertura.",
        "en": "Present, directly and objectively, the current water quality status of the {trecho} stretch of the Tietê River in {ano}, based on real CETESB network measurements, complemented by a simulated series as fill-in for years without that coverage.",
        "fr": "Présenter, de manière directe et objective, l'état actuel de la qualité de l'eau du tronçon {trecho} du fleuve Tietê en {ano}, sur la base de mesures réelles du réseau CETESB, complétées par une série simulée en remplissage pour les années sans cette couverture.",
        "es": "Presentar, de forma directa y objetiva, el estado actual de la calidad del agua del tramo {trecho} del Río Tietê en el año {ano}, con base en mediciones reales de la red CETESB, complementadas por una serie simulada como relleno en los años sin esa cobertura.",
    },
    "pdf.a.objetivo_texto_todos": {
        "pt": "Apresentar, de forma direta e objetiva, o estado atual da qualidade da água de todos os trechos monitorados do Rio Tietê no ano de {ano}, com base em medições reais da rede CETESB, complementadas por uma série simulada de preenchimento nos anos sem essa cobertura.",
        "en": "Present, directly and objectively, the current water quality status of all monitored stretches of the Tietê River in {ano}, based on real CETESB network measurements, complemented by a simulated series as fill-in for years without that coverage.",
        "fr": "Présenter, de manière directe et objective, l'état actuel de la qualité de l'eau de tous les tronçons surveillés du fleuve Tietê en {ano}, sur la base de mesures réelles du réseau CETESB, complétées par une série simulée en remplissage pour les années sans cette couverture.",
        "es": "Presentar, de forma directa y objetiva, el estado actual de la calidad del agua de todos los tramos monitoreados del Río Tietê en el año {ano}, con base en mediciones reales de la red CETESB, complementadas por una serie simulada como relleno en los años sin esa cobertura.",
    },
    ...
    "cn.nota": {
        "pt": "Simulação via modelo de agentes (Mesa) + balanço hídrico + Streeter-Phelps, com submodelos estendidos ancorados em médias reais 2012-2024 da CETESB. O IQA exibido segue o modelo OFICIAL CETESB/NSF (9 parâmetros — OD, coliformes termotolerantes, pH, DBO, temperatura, nitrogênio total, fósforo total, turbidez e sólidos totais —, combinados por produtório ponderado); o Índice Biótico e o índice de Metais/Tóxicos são proxies ilustrativos que combinam os demais parâmetros simulados, não medições diretas de campo. Cada trecho do rio é simulado de forma independente, sem propagação de carga entre trechos.",
        "en": "Simulation via agent-based model (Mesa) + water balance + Streeter-Phelps, with extended submodels anchored to real 2012-2024 CETESB averages. The displayed WQI follows the OFFICIAL CETESB/NSF model (9 parameters — DO, thermotolerant coliforms, pH, BOD, temperature, total nitrogen, total phosphorus, turbidity and total solids —, combined by weighted product); the Biotic Index and the Metals/Toxics index are illustrative proxies combining the other simulated parameters, not direct field measurements. Each river stretch is simulated independently, with no load propagation between stretches.",
        "fr": "Simulation via un modèle multi-agents (Mesa) + bilan hydrique + Streeter-Phelps, avec des sous-modèles étendus ancrés sur des moyennes réelles CETESB 2012-2024. L'IQE affiché suit le modèle OFFICIEL CETESB/NSF (9 paramètres — OD, coliformes thermotolérants, pH, DBO, température, azote total, phosphore total, turbidité et solides totaux —, combinés par produit pondéré) ; l'Indice Biotique et l'indice Métaux/Toxiques sont des proxys illustratifs combinant les autres paramètres simulés, pas des mesures directes de terrain. Chaque tronçon du fleuve est simulé indépendamment, sans propagation de charge entre les tronçons.",
        "es": "Simulación vía modelo basado en agentes (Mesa) + balance hídrico + Streeter-Phelps, con submodelos extendidos anclados en promedios reales 2012-2024 de la CETESB. El ICA mostrado sigue el modelo OFICIAL CETESB/NSF (9 parámetros — OD, coliformes termotolerantes, pH, DBO, temperatura, nitrógeno total, fósforo total, turbidez y sólidos totales —, combinados por producto ponderado); el Índice Biótico y el índice de Metales/Tóxicos son proxies ilustrativos que combinan los demás parámetros simulados, no mediciones directas de campo. Cada tramo del río se simula de forma independiente, sin propagación de carga entre tramos.",
    },
    ...
    # ACHADO (2026-09): o Relatório Automático (trecho/todos os trechos) reaproveitava a chave
    # acima (correta só para o relatório de Cenário/ABM) na folha de rosto — mas usa medição
    # REAL da CETESB (+ fallback simulado), não uma simulação computacional. Esta chave é o
    # texto correto para esse relatório, ver `_renderizar_documento_completo_inicio`.
    "rel.b.natureza_texto": {
        "pt": "Relatório técnico-científico gerado a partir de medições reais da rede CETESB, complementadas por uma série simulada de preenchimento nos anos sem essa cobertura.",
        "en": "Technical-scientific report generated from real CETESB network measurements, complemented by a simulated series as fill-in for years without that coverage.",
        "fr": "Rapport technico-scientifique généré à partir de mesures réelles du réseau CETESB, complétées par une série simulée en remplissage pour les années sans cette couverture.",
        "es": "Informe técnico-científico generado a partir de mediciones reales de la red CETESB, complementadas por una serie simulada como relleno en los años sin esa cobertura.",
    },
